...
NSString *visibleContentsForLeafNode(xmlNode *leaf) {	
	if(node_is_a(leaf, "br")) {
		...
		// whitespace chunks are usually rendered as a single space in HTML
		NSMutableString *nodeContent = [NSMutableString stringWithCString:(const char *)leaf->content encoding:NSUTF8StringEncoding];
		[nodeContent replaceOccurrencesOfRegex:@"\\s+" withString:@" "];
		return nodeContent;
	} else {
		// no content
		return @"";
...
NSString *visibleContentsForNode(xmlNode *node, int recursive) {	
	NSMutableString *content = [NSMutableString string];
	
	xmlNode *sibling = node;
	while(sibling) {
		[content appendString:visibleContentsForLeafNode(sibling)];
		
		if(recursive && sibling->children)
			[content appendString:visibleContentsForNode(sibling->children,1)];
		
		sibling = sibling->next;
	}
	
	// strip some whitespace where appropriate
	[content replaceOccurrencesOfRegex:@"^\\s*" withString:@""];
	[content replaceOccurrencesOfRegex:@"[\\x20\\t]*\\n[\\x20\\t]*" withString:@"\n"];
	[content replaceOccurrencesOfRegex:@"\\s*$" withString:@""];	
	
	return content;	
}

-(NSString*)visibleContents {
	NSString *result = visibleContentsForNode(_node->children,0);
	return ([result length] > 0 ? result : nil);
}

-(NSString*)allVisibleContents {
	NSString *result = visibleContentsForNode(_node,1);
	return ([result length] > 0 ? result : nil);
}
...
{	
	...
	htmlNodeDumpFormatOutput(buf, node->doc, node, (const char *)node->doc->encoding, 0);
